...
export async function createTopicController(_app, request, reply) {
    const { id } = request.params;
    const { userId } = request.user;
    const discipline = await assertDisciplineOwnership(id, userId, reply);
    if (!discipline)
...
export async function updateTopicController(_app, request, reply) {
...
export async function deleteTopicController(_app, request, reply) {
    const { id, topicId } = request.params;
    const { userId } = request.user;
    const discipline = await assertDisciplineOwnership(id, userId, reply);
    if (!discipline)
...
export async function toggleTopicController(_app, request, reply) {
    const { id, topicId } = request.params;
    const { userId } = request.user;
    const discipline = await assertDisciplineOwnership(id, userId, reply);
    if (!discipline)

...
export async function listEvaluationsController(_app, request, reply) {
...
export async function createEvaluationController(_app, request, reply) {
    const { id } = request.params;
    const { userId } = request.user;
    const discipline = await assertDisciplineOwnership(id, userId, reply);
    if (!discipline)
...
export async function updateEvaluationController(_app, request, reply) {
    const { id, evalId } = request.params;
    const { userId } = request.user;
    const discipline = await assertDisciplineOwnership(id, userId, reply);
    if (!discipline)
        return;
...
export async function deleteEvaluationController(_app, request, reply) {
    const { id, evalId } = request.params;
    const { userId } = request.user;
    const discipline = await assertDisciplineOwnership(id, userId, reply);
    if (!discipline)
...
export async function toggleEvaluationController(_app, request, reply) {
    const { id, evalId } = request.params;
    const { userId } = request.user;
    const discipline = await assertDisciplineOwnership(id, userId, reply);
    if (!discipline)

...
type Evaluation = { completed: boolean }

export function calculateProgress(tasks: Task[], evaluations: Evaluation[]): number {
  const hasTasks = tasks.length > 0
  const hasEvals = evaluations.length > 0

  if (!hasTasks && !hasEvals) return 0
  ...
  if (taskScore === null) return evalScore!
  if (evalScore === null) return taskScore

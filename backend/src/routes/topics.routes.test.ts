...
async function createDiscipline(app: FastifyInstance, token: string, name = 'Cálculo I') {
...
async function createTopic(
  ...
  token: string,
  disciplineId: string,
  payload: Record<string, unknown> = { title: 'Lista 1', description: 'Exercícios cap. 2' },
) {
  const res = await app.inject({
    method: 'POST',
